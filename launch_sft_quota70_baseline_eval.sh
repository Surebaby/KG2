#!/usr/bin/env bash
# Full pipeline EM/F1 for the quota70 SFT checkpoint (branch A), under the SAME
...
#
#   bash launch_sft_quota70_baseline_eval.sh
set -euo pipefail
cd "$(dirname "$0")"
...
exec python scripts/eval/run_kg_proweight.py \
  --checkpoint /home/zjulab/kgpaper/checkpoints/sft_legacy_repaired_v2_quota70_hard_seed42_no_text_head/final \
  ...
  --rerank 10 \
  --gpu_id 0 \
  --save_root /home/zjulab/kgpaper/outputs/sft_quota70_baseline_eval
