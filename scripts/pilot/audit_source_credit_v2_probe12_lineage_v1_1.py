...
OUTPUT = ROOT / "outputs/audits/ppo_a_probe12_independent_lineage_audit_20260906_v1_1"
...
def audit(out=OUTPUT):
    ...
    check("prelaunch_remote_sha_verification",remote["status"]=="IMMUTABLE_ASSETS_AND_TRAINING_MASK_VERIFIED_NO_TRAINING" and all(v["match"] for v in remote["files"].values()))
    ...
    report={"schema_version":"source-credit-v2-probe12-independent-lineage-audit-v1-1","experiment_id":"SOURCE-CREDIT-V2-PROBE12-INDEPENDENT-LINEAGE-AUDIT-20260906-V1-1","created_at_utc":datetime.now(timezone.utc).isoformat(),"status":"PASS_ENGINEERING_LINEAGE_ONLY" if passed else "FAIL",
    ...
    report["audit_correction"]={"parent_report":binding(ROOT/"outputs/audits/ppo_a_probe12_independent_lineage_audit_20260906_v1/report.json"),"parent_code":binding(ROOT/"scripts/pilot/audit_source_credit_v2_probe12_lineage_v1.py"),"reason":"Match the exact existing successful remote-assets status; all asset hashes were already true. No training/reward/decision rule changed."}
    report["next_step_recommendation"]={"recommendation":"Proceed to a separately frozen bounded A-smoke600 under the existing cost guard after200 trajectories; no recalibration or new SFT is required by this engineering evidence.","fresh_health_status":"FAIL","current_scope_allows600":False,"automatically_launched":False,"evidence":"All three domains completed the real PPO path; positive alpha/Graph occurred on3 valid graph candidates, Text on34 steps, explicit SFT KL started at0, replay executed, and checkpoint identity changed.","limits":"Not an EM/F1 improvement claim or alpha superiority claim; preserve current learning rate, mini-batch, reward, alpha and data for the next comparable run."}
