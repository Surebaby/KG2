"""Run only the frozen A-probe12 and retain its exit status and GPU observations."""
...
def main():
    ...
    run = project / "outputs/launches/ppo_a_probe12_scoped_20260906_v1_supervision"
    output = project / "outputs/ppo_mixed4_answer_format_v2_a_probe12_scoped_seed42_20260906_v1"
    log_path = project / "outputs/launches/ppo_a_probe12_scoped_20260906_v1.log"
    ...
    launch = {"experiment_id": "SOURCE-CREDIT-V2-A-PROBE12-GPU-SUPERVISED-20260906-V1",
              "created_at_utc": datetime.now(timezone.utc).isoformat(),
              "supervisor_pid": os.getpid(), "training_pid": child.pid, "command": command,
              "config_sha256": hashlib.sha256((project / config).read_bytes()).hexdigest(),
              "supervisor_sha256": hashlib.sha256(Path(__file__).read_bytes()).hexdigest(),
              "release_manifest_sha256": hashlib.sha256((project / "outputs/audits/ppo_remote_prelaunch_20260906_v1/manifest.json").read_bytes()).hexdigest(),
              "scope_sha256": hashlib.sha256((project / "outputs/calibration/source_credit_gate_v2_probe12_scoped_20260906_v1/scope.json").read_bytes()).hexdigest(),
              "trajectory_limit": 12, "automatic_restart": False, "automatic_smoke_or_full": False}
    ...
    while True:
        ...
        record = {"observed_at_utc": datetime.now(timezone.utc).isoformat(), "training_pid": child.pid,
                  ...
                  "exit_code": code, "gpu_csv": gpu.stdout.strip(), "gpu_query_returncode": gpu.returncode,
