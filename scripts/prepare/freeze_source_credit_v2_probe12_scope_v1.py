...
PARENT_CONFIG = "configs/training/phase3_ppo_mixed4_answer_format_v2_a_probe_seed42.yaml"
CONFIG = "configs/training/phase3_ppo_mixed4_answer_format_v2_a_probe12_scoped_seed42.yaml"
OUTPUT = "outputs/calibration/source_credit_gate_v2_probe12_scoped_20260906_v1"
...
def freeze(*, out=OUTPUT, config=CONFIG):
    ...
    code_names |= {"scripts/train/phase3_ppo.py", "scripts/train/_split_args.py", "scripts/prepare/resolve_phase3_ppo_runtime_config.py",
                   "scripts/prepare/freeze_source_credit_v2_probe12_scope_v1.py", "scripts/sourcegate_python.sh"}
    code_names |= config_files(config)
    code = {name: bind(ROOT / name) for name in sorted(code_names)}
    bound_scope = {"schema_version": scope.SCHEMA, "scope": "complete_A_probe12_only",
        "experiment_id": "SOURCE-CREDIT-V2-FRESH-UTILITY-A-PROBE12-SEED42-20260906-V1",
        ...
        "limits": {"trajectories":12,"prompt_groups":3,"rollouts_per_prompt":4,"ppo_batches":3,
                   "automatic_resume":False,"automatic_smoke_or_full":False},
        ...
        "scope_does_not_establish": ["alpha superiority", "PPO performance improvement", "health PASS", "smoke/full clearance"],
        "model_updates_at_freeze": 0, "gold_values_opened": False}
    ...
    artifact.update(experiment_id=bound_scope["experiment_id"], training_clearance=True, independent_confirmation_clearance=True,
                    ppo_launch_clearance=False, training_clearance_scope="complete_A_probe12_only",
    ...
    report = {"schema_version": scope.SCHEMA, "status": "CPU_READY_BOUNDED_A_PROBE12_NOT_STARTED",
        ...
        "config_changed_fields": sorted(changed), "training_started": False, "optimizer_updates":0,
