"""Fail-closed execution scope for one authorized, complete A-probe12.
...
SCHEMA = "source-credit-v2-bounded-a-probe12-scope-v1"
...
PARENT_CONFIG_SCIENTIFIC_SHA256 = "4be6db85dd615ee63314fd70159e029f6fa1805e48e47412e072e01e54497e80"
...
def read_scope(data, artifact_path, *, root=None):
    ...
    require(scope.get("schema_version") == SCHEMA and scope.get("scope") == "complete_A_probe12_only",
...
def validate_probe_scope(data, cfg, artifact_path, mask, *, root=None):
    ...
    required = {"total_steps": 12, "batch_size": 4, "rollouts_per_prompt": 4,
                ...
                "proofkg_process_version": "v2_3", "log_with": "tensorboard"}
    require(all(actual.get(k) == v and type(actual.get(k)) is type(v) for k, v in required.items()),
            "scoped child permits only complete A/12 trajectories/K4/batch4")
    ...
    require(data.get("training_clearance") is True and data.get("independent_confirmation_clearance") is True
            ...
            and data.get("training_clearance_scope") == "complete_A_probe12_only", "scoped child flags contradict limited permission")
    ...
    require(len(rows) == 12 and [x["rollout_index"] for x in rows] == list(range(1,13)), "fixed schedule must have exactly12 trajectories")
    for start in range(0,12,4):
        require(len({(x["dataset"], x["qid"]) for x in rows[start:start+4]}) == 1, "schedule violates K4 groups")
    require(Counter(x["dataset"] for x in rows) == {"hotpotqa":4,"2wikimultihopqa":4,"musique":4}, "fixed three-domain probe schedule differs")
    required_code = {"kgproweight/reward/source_gate_probe_scope_v1.py", "kgproweight/reward/source_credit_gate_v2.py",
                     "kgproweight/training/phase3_ppo.py", "scripts/train/phase3_ppo.py", "scripts/train/_split_args.py",
                     "scripts/prepare/resolve_phase3_ppo_runtime_config.py", "scripts/prepare/freeze_source_credit_v2_probe12_scope_v1.py",
                     "scripts/sourcegate_python.sh", refs["config"]["path"], refs["parent_config"]["path"]}
    ...
    require(scope["limits"] == {"trajectories":12,"prompt_groups":3,"rollouts_per_prompt":4,"ppo_batches":3,
                                "automatic_resume":False,"automatic_smoke_or_full":False}, "bounded execution limits changed")
    return {"scope": "complete_A_probe12_only", "runtime_config_sha256": digest(actual), "trajectory_limit": 12,
            "utility_report_sha256": UTILITY_REPORT_SHA256, "health_status": "FAIL", "matched600_clearance": False}


def validate_probe_execution_paths(gate, cfg):
...
def load_referenced_probe_before_dispatch(cfg):
    """A referenced limited child remains limited when CLI reward flags change."""
    path = Path(cfg.source_gate_calibration_path or "")
    if not path.is_file():
        return None
    data = json.loads(path.read_text())
    if SCOPE_FIELD not in data and "training_clearance_scope" not in data:
        return None
    ...
    validate_probe_execution_paths(gate, cfg)
    return gate
