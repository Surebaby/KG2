...
class _HFLogprobEntry:
    """Mimics vLLM logprob entries (attribute ``logprob``) for SelfRAGPipeline."""
...
def _hf_step_logprobs_dict(logits_row, chosen_id: int, topn):
    """Build ``{token_id: _HFLogprobEntry}`` for one generation step (top-``topn`` + chosen id)."""
    import torch

    log_probs = torch.log_softmax(logits_row.float(), dim=-1)
    n_vocab = int(log_probs.shape[-1])
    k = n_vocab if topn is None else min(int(topn), n_vocab)
    values, indices = torch.topk(log_probs, k=k, dim=-1)
    d = {int(tid): _HFLogprobEntry(float(lp)) for tid, lp in zip(indices.tolist(), values.tolist())}
    cid = int(chosen_id)
    if cid not in d:
        d[cid] = _HFLogprobEntry(float(log_probs[cid].item()))
    return d


class _HFCompletionOutput:
    """Single completion compatible with ``SelfRAGPipeline`` / vLLM ``RequestOutput``."""

    def __init__(self, text: str, token_ids: list, logprobs: list, cumulative_logprob: float):
        self.text = text
...
class _HFRequestOutput:
    ...
    def __init__(self, completion: _HFCompletionOutput):
        self.outputs = [completion]


def _hf_request_output_from_generate(
    tokenizer,
    outputs,
    inputs,
    batch_i: int,
    stop_sym,
    skip_decode_special: bool,
    n_logprobs,
):
    """Build one vLLM-shaped ``_HFRequestOutput`` from HF ``generate`` outputs."""
    import torch

    generated_sequence = outputs.sequences[batch_i]
    input_ids = inputs["input_ids"][batch_i]
    gen_start = int(input_ids.shape[0])
    new_ids_tensor = generated_sequence[gen_start:]
    new_ids_list = [int(x) for x in new_ids_tensor.tolist()]

    logprobs_per_step = []
    cum = 0.0
    for t, tid in enumerate(new_ids_list):
        if not outputs.scores or t >= len(outputs.scores):
            logprobs_per_step.append({})
            continue
        raw = outputs.scores[t]
        if raw.dim() == 3:
            logits_row = raw[batch_i, -1]
        else:
            logits_row = raw[batch_i]
        d = _hf_step_logprobs_dict(logits_row, tid, n_logprobs)
        logprobs_per_step.append(d)
        if tid in d:
            cum += float(d[tid].logprob)

    text = tokenizer.decode(
        generated_sequence,
        skip_special_tokens=skip_decode_special,
    ...
    completion = _HFCompletionOutput(new_text, new_ids_list, logprobs_per_step, cum)
    return _HFRequestOutput(completion)
...
class HFCausalLMGenerator(BaseGenerator):
    ...
    def generate(
        ...
        **params,
        ...
        return_raw_output = bool(generation_params.pop("return_raw_output", False))
        n_logprobs = generation_params.pop("logprobs", None)
        skip_decode_special = generation_params.pop("skip_special_tokens", True)
        for _k in ("frequency_penalty", "presence_penalty", "best_of", "top_logprobs", "logit_bias"):
            generation_params.pop(_k, None)
        ...
        raw_outputs = [] if return_raw_output else None
        ...
        for idx in trange(0, len(input_list), batch_size, desc="Generation process: "):
            ...
            if return_dict:
                ...
                if batch_generated_token_ids.shape[1] < generation_params.get(
                    "max_new_tokens", batch_generated_token_ids.shape[1]
                ):
                    real_batch_size, num_generated_tokens = batch_generated_token_ids.shape
                    padding_length = generation_params.get(
            ...
            if return_raw_output:
                for i in range(len(outputs.sequences)):
                    raw_outputs.append(
                        _hf_request_output_from_generate(
                            self.tokenizer,
                            outputs,
                            inputs,
                            i,
