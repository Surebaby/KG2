...
import numpy as np
from flashrag.utils.progress_log import StageTimer, log_stage
import torch
...
class KGTraceRefiner(BaseRefiner):
    def __init__(self, config, retriever=None, generator=None):
        ...
        self.doc_generate_batch_size = int(self.kg_setting["doc_generate_batch_size"])
        if "reference_template" in self.kg_setting:
            self.reference_template = self.kg_setting["reference_template"]
        elif "reference_template" in config:
            self.reference_template = config["reference_template"]
        else:
    ...
    def extract_document_triples(self, queries, retrieval_results):
        ...
        if len(docs_content) > 0:
            ...
            log_stage(
                "KG-TRACE: LLM triple extraction on %d unique documents (%d prompts) ...",
                len(docs_content),
                len(prompts),
            )
            with StageTimer(f"triple extraction ({len(prompts)} prompts)"):
                outputs = []
                bs = max(1, self.doc_generate_batch_size)
                for start in range(0, len(prompts), bs):
                    batch_prompts = prompts[start : start + bs]
                    outputs.extend(self.generator.generate(batch_prompts, max_tokens=512))
    ...
    def get_reasoning_chain(self, all_query, all_doc_triples, triple_to_doc_ids):
        ...
        for query, doc_triples, generating_chain_examplars, final_chain_examplars, triple_doc_id_list in tqdm(
            ...
            if num_total_triples == 0:
                log_stage(
                    "KG-TRACE: no triples for query (cached docs empty or parse failed); "
                    "skip reasoning-chain beam search for this item."
                )
            ...
            for j in range(self.max_chain_length):
                ...
                if not new_paths:
                    # All beam branches rejected (e.g. "No choice in generated results")
                    break
    ...
    def format_reference(self, retrieval_result):
        ...
        for idx, doc_item in enumerate(retrieval_result):
            ...
            title = content.split("\n")[0]
            text = "\n".join(content.split("\n")[1:])
            ref_tpl = getattr(self, "reference_template", None)
            if ref_tpl is not None:
                format_reference += ref_tpl.format(idx=idx, title=title, text=text)
            else:
    ...
    def batch_run(self, dataset):
        ...
        n_q = len(all_query)
        log_stage("KG-TRACE refiner: %d queries — begin triple extraction ...", n_q)
        with StageTimer(f"extract document triples ({n_q} queries)"):
        ...
        log_stage("KG-TRACE: triple extraction finished.")

        log_stage("KG-TRACE: begin reasoning-chain construction for %d queries ...", n_q)
        with StageTimer(f"reasoning chains ({n_q} queries)"):
        ...
        log_stage("KG-TRACE: reasoning-chain construction finished.")
        ...
        if self.context_type == "triples":
            for query_chain_result, query_retrieval_results in zip(
                ...
                if all_triple_text:
                    refine_text = "\n".join(
                        ["{}. {}".format(i + 1, text) for i, text in enumerate(all_triple_text)]
                    )
                else:
                    # Fallback: use retrieved passages when triple extraction / chain is empty
                    refine_text = self.format_reference(
                        query_retrieval_results[: self.n_context]
                    )
            ...
            for query_chain_result, query_retrieval_results in zip(reasoning_chain_result, retrieval_results):
                ...
                final_doc_idx = [query_doc_to_idx[doc_id] for doc_id, _ in ranked_chains_doc_id]
                final_doc_list = [query_retrieval_results[idx] for idx in final_doc_idx]
                if not final_doc_list:
                    final_doc_list = query_retrieval_results[: self.n_context]
