...
def load_docs(corpus, doc_idxs: List[int], show_progress: bool = True):
    """Load corpus rows by index. Deduplicates indices first (large speedup on HF datasets)."""
    if doc_idxs is None:
        return []
    if isinstance(doc_idxs, np.ndarray):
    ...
    log = logging.getLogger("flashrag.progress")
    unique_idxs = list(dict.fromkeys(int(i) for i in doc_idxs))
    n_total, n_unique = len(doc_idxs), len(unique_idxs)

    if n_unique > 50 and show_progress:
        log.info(
            "Corpus lookup: %d document slots, %d unique indices (loading unique only) ...",
            n_total,
            n_unique,
        )
        cache = {}
        for idx in tqdm(unique_idxs, desc="Corpus lookup", unit="doc"):
            cache[idx] = corpus[idx]
        return [cache[int(i)] for i in doc_idxs]
