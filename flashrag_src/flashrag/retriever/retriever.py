...
if get_device() == "cpu":
    ...
    # Use all CPU cores for FAISS when the index stays on CPU (typical for ~45GB Flat indexes).
    try:
        import os
        faiss.omp_set_num_threads(int(os.environ.get("FAISS_NUM_THREADS", os.cpu_count() or 8)))
    except Exception:
        pass
...
class DenseRetriever(BaseTextRetriever):
    ...
    def load_index(self):
        ...
        if self.use_faiss_gpu:
            ...
            co.useFloat16 = True
            co.shard = True
            log_stage("Moving FAISS index to GPU (requires enough VRAM; ~45GB Flat may OOM on single GPU) ...")
            self.index = faiss.index_cpu_to_all_gpus(self.index, co=co)
        index_kind = type(self.index).__name__
        log_stage(
            "FAISS ready: ntotal=%s, dim=%s, backend=%s, faiss_gpu=%s. "
            "Flat+CPU brute-force on thousands of queries vs ~15M vectors often takes 15-60+ min; "
            "use --faiss_gpu only if VRAM allows, or build an IVF index (see OPERATION_GUIDE Step 1).",
            f"{ntotal:,}",
            dim,
            index_kind,
            self.use_faiss_gpu,
    ...
    def _batch_search(self, query: List[str], num: int = None, return_score=False):
        ...
        n_queries = len(query)
        log_stage(
            "Dense retrieval: encoding %d queries (batch_size=%d, topk=%d) ...",
            n_queries,
            batch_size,
            num,
        )
        emb = self.encoder.encode(query, batch_size=batch_size, is_query=True)
        faiss_chunk = getattr(self, "_faiss_search_batch_size", 64)
        with StageTimer(f"FAISS search ({n_queries} queries, topk={num}, chunk={faiss_chunk})"):
            scores, idxs = self._faiss_search_chunked(emb, num, faiss_chunk)
        scores = scores.tolist()
        idxs = idxs.tolist()

        flat_idxs = [idx for sublist in idxs for idx in sublist]
        with StageTimer(f"Corpus document load ({len(flat_idxs)} slots)"):
            results = load_docs(self.corpus, flat_idxs, show_progress=not self.silent)
        results = [results[i * num : (i + 1) * num] for i in range(len(idxs))]
        log_stage("Dense retrieval finished for %d queries.", n_queries)
    ...
    def _faiss_search_chunked(self, emb: np.ndarray, k: int, chunk_size: int):
        """Run FAISS search in chunks so tqdm shows progress on large query batches."""
        n = emb.shape[0]
        if n <= chunk_size:
        ...
        if not self.silent:
            iterator = tqdm(iterator, desc=desc, total=(n + chunk_size - 1) // chunk_size)
        for start in iterator:
            end = min(start + chunk_size, n)
            batch_scores, batch_idxs = self.index.search(emb[start:end], k=k)
            all_scores.append(batch_scores)
            all_idxs.append(batch_idxs)
        return np.vstack(all_scores), np.vstack(all_idxs)
