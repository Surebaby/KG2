...
class SequentialPipeline(BasicPipeline):
    ...
    def run(self, dataset, do_eval=True, pred_process_fun=None):
        ...
        n = len(input_query)
        log_stage("SequentialPipeline: %d samples — retrieval starting ...", n)
        with StageTimer(f"batch retrieval ({n} queries)"):
        ...
        if self.refiner:
            ...
            if "llmlingua" in self.refiner.name and input_prompt_flag:
                ...
                log_stage(
                    "SequentialPipeline: refiner '%s' on %d samples ...",
                    getattr(self.refiner, "name", type(self.refiner).__name__),
                    n,
                )
                with StageTimer(f"refiner {getattr(self.refiner, 'name', 'refiner')}"):
        ...
        if self.refiner:
            del self.refiner
        log_stage("SequentialPipeline: LLM generation on %d prompts ...", len(input_prompts))
        with StageTimer(f"generation ({len(input_prompts)} prompts)"):
            pred_answer_list = self.generator.generate(input_prompts)
        dataset.update_output("pred", pred_answer_list)
