    ...
    def postprocess_agent_response(self, response):
        ...
        for steps in parsed_codes:
            for step_idx, step in steps.items():
                ...
                action_code = self.extract_code(action_raw).strip()
                if not action_code:
                    continue

                action = None
                # Prefer safe parsing first; keep a narrow eval fallback for legacy traces.
                for parser in (json.loads, ast.literal_eval):
                    try:
                        action = parser(action_code)
                        break
                    except Exception:
                        continue
                if action is None:
                    try:
                        action = eval(action_code, {"__builtins__": {}}, {})
                    except Exception:
                        continue
